...
describe('YAML Validation Tests', () => {
    ...
    describe('Unused anchors diagnostics', () => {
        it('should report unused anchor', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            chai_1.expect(result).is.not.empty;
            chai_1.expect(result.length).to.be.equal(1);
            chai_1.expect(result[0]).deep.equal(verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&bar"', '&bar', 0, 5, 0, 9));
        }));
        it('should not report used anchor', () => __awaiter(void 0, void 0, void 0, function* () {
            const yaml = 'foo: &bar bar\nfff: *bar';
            const result = yield parseSetup(yaml);
            chai_1.expect(result).is.empty;
        }));
...
&aa ff: 
  ...
  - e`;
            ...
            chai_1.expect(result).to.include.deep.members([
                verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&bar"', '&bar', 0, 5, 0, 9),
                verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&a"', '&a', 4, 2, 4, 4),
                verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&aa"', '&aa', 5, 0, 5, 3),
                verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&e"', '&e', 8, 4, 8, 6),
            ]);
        }));
    });
});
