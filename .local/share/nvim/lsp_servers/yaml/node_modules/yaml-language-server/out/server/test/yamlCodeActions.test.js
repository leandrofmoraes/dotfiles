...
describe('CodeActions Tests', () => {
    ...
    describe('Remove Unused Anchor', () => {
        it('should generate proper action', () => {
            const doc = testHelper_1.setupTextDocument('foo: &bar bar\n');
            const diagnostics = [verifyError_1.createUnusedAnchorDiagnostic('Unused anchor "&bar"', '&bar', 0, 5, 0, 9)];
            const params = {
                context: vscode_languageserver_1.CodeActionContext.create(diagnostics),
                range: undefined,
                textDocument: vscode_languageserver_1.TextDocumentIdentifier.create(testHelper_1.TEST_URI),
            ...
            expect(result[0].edit.changes[testHelper_1.TEST_URI]).deep.equal([vscode_languageserver_1.TextEdit.del(vscode_languageserver_1.Range.create(0, 5, 0, 10))]);
        });
        it('should delete all whitespace after unused anchor', () => {
            ...
            const result = actions.getCodeAction(doc, params);
            expect(result[0].title).to.be.equal('Delete unused anchor: &bar');
            expect(result[0].edit.changes[testHelper_1.TEST_URI]).deep.equal([vscode_languageserver_1.TextEdit.del(vscode_languageserver_1.Range.create(0, 5, 0, 13))]);
        });
    });
});
