    ...
    class YamlCompletion {
        ...
        doComplete(document, position, isKubernetes = false) {
            return __awaiter(this, void 0, void 0, function* () {
                const result = node_1.CompletionList.create([], false);
                ...
                if (node && yaml_1.isScalar(node) && node.value === 'null') {
                    const nodeStartPos = document.positionAt(node.range[0]);
                    nodeStartPos.character += 1;
                    const nodeEndPos = document.positionAt(node.range[2]);
                    nodeEndPos.character += 1;
                    overwriteRange = node_1.Range.create(nodeStartPos, nodeEndPos);
                }
                else if (node && yaml_1.isScalar(node) && node.value) {
                    const start = document.positionAt(node.range[0]);
                    if (offset > 0 && start.character > 0 && document.getText().charAt(offset - 1) === '-') {
                        start.character -= 1;
                    }
                    overwriteRange = node_1.Range.create(start, document.positionAt(node.range[1]));
                }
                else {
                ...
                try {
                    ...
                    if (!schema || schema.errors.length) {
                        if (position.line === 0 && position.character === 0 && !modelineUtil_1.isModeline(lineContent)) {
                    ...
                    if (modelineUtil_1.isModeline(lineContent) || astUtils_1.isInComment(doc.tokens, offset)) {
                        ...
                        if (schemaIndex !== -1 && schemaIndex + '$schema='.length <= position.character) {
                            this.schemaService.getAllSchemas().forEach((schema) => {
                                ...
                                const schemaIdCompletion = {
                                    ...
                                    insertTextFormat: node_1.InsertTextFormat.PlainText,
                                    insertTextMode: node_1.InsertTextMode.asIs,
                                };
                    ...
                    if (!node) {
                        if (!currentDoc.internalDocument.contents || yaml_1.isScalar(currentDoc.internalDocument.contents)) {
                        ...
                        if (lineContent.length === 0) {
                            ...
                            if (parent) {
                                if (yaml_1.isScalar(node)) {
                                    if (node.value) {
                                        if (yaml_1.isPair(parent)) {
                                            if (parent.value === node) {
                                                if (lineContent.trim().length > 0 && lineContent.indexOf(':') < 0) {
                                                    const map = this.createTempObjNode(currentWord, node, currentDoc);
                                                    if (yaml_1.isSeq(currentDoc.internalDocument.contents)) {
                                                        const index = astUtils_2.indexOf(currentDoc.internalDocument.contents, parent);
                                        ...
                                        if (yaml_1.isPair(parent)) {
                                            if (parent.key === node) {
                                                ...
                                                if (yaml_1.isNode(parent.key) && parent.key.range) {
                                                    const parentParent = currentDoc.getParent(parent);
                                                    if (foundByClosest && parentParent && yaml_1.isMap(parentParent) && astUtils_1.isMapContainsEmptyPair(parentParent)) {
                                                        node = parentParent;
                                                    }
                                                    else {
                                                        ...
                                                        if (position.character > parentPosition.character && position.line !== parentPosition.line) {
                                                            const map = this.createTempObjNode(currentWord, node, currentDoc);
                                                            if (parentParent && (yaml_1.isMap(parentParent) || yaml_1.isSeq(parentParent))) {
                                ...
                                else if (yaml_1.isMap(node)) {
                                    if (!foundByClosest && lineContent.trim().length === 0 && yaml_1.isSeq(parent)) {
                                ...
                                for (const pair of node.items) {
                                    if (yaml_1.isNode(pair.value) && pair.value.range && pair.value.range[0] === offset + 1) {
                                        node = pair.value;
                                    }
                                }
                            }
                    ...
                    if (node && yaml_1.isMap(node)) {
                        // don't suggest properties that are already present
                        const properties = node.items;
                        for (const p of properties) {
                            if (!currentProperty || currentProperty !== p) {
                                if (yaml_1.isScalar(p.key)) {
                                    proposed[p.key.value + ''] = node_1.CompletionItem.create(existingProposeItem);
                                }
                            }
                        }
        ...
        addPropertyCompletions(schema, doc, node, originalNode, separatorAfter, collector, textBuffer, overwriteRange) {
            ...
            for (const schema of matchingSchemas) {
                if (((schema.node.internalNode === node && !matchOriginal) || schema.node.internalNode === originalNode) &&
                                ...
                                if (Object.prototype.hasOwnProperty.call(schemaProperties, key)) {
                                    ...
                                    if (typeof propertySchema === 'object' && !propertySchema.deprecationMessage && !propertySchema['doNotSuggest']) {
                                        ...
                                        if (propertySchema.type === 'array' &&
                                            (pair = node.items.find((it) => yaml_1.isScalar(it.key) &&
                                                it.key.range &&
                                                it.key.value === key &&
                                                yaml_1.isScalar(it.value) &&
                                                !it.value.value &&
                    ...
                    if (nodeParent && yaml_1.isSeq(nodeParent) && schema.schema.type !== 'object') {
        ...
        getValueCompletions(schema, doc, node, offset, document, collector, types) {
            ...
            if (yaml_1.isPair(node)) {
                ...
                parentKey = yaml_1.isScalar(node.key) ? node.key.value + '' : null;
                node = doc.getParent(node);
            }
            if (node && (parentKey !== null || yaml_1.isSeq(node))) {
                ...
                for (const s of matchingSchemas) {
                    if (s.node.internalNode === node && !s.inverted && s.schema) {
                        if (s.schema.items) {
                            ...
                            if (yaml_1.isSeq(node) && node.items) {
            ...
            if (this.supportsMarkdown === undefined) {
                ...
                this.supportsMarkdown =
                    completion &&
                        ...
                        completion.completionItem.documentationFormat.indexOf(node_1.MarkupKind.Markdown) !== -1;
            }
