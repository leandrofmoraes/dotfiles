...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "vscode-languageserver", "../../commands", "path", "../utils/textBuffer", "../parser/jsonParser07", "../utils/strings"], factory);
    }
        ...
        getUnusedAnchorsDelete(diagnostics, document) {
            ...
            for (const diag of diagnostics) {
                if (diag.message.startsWith('Unused anchor') && diag.source === jsonParser07_1.YAML_SOURCE) {
                    ...
                    const range = vscode_languageserver_1.Range.create(diag.range.start, diag.range.end);
                    const lineContent = buffer.getLineContent(range.end.line);
                    const lastWhitespaceChar = strings_1.getFirstNonWhitespaceCharacterAfterOffset(lineContent, range.end.character);
                    range.end.character = lastWhitespaceChar;
                    const action = vscode_languageserver_1.CodeAction.create(`Delete unused anchor: ${name}`, createWorkspaceEdit(document.uri, [vscode_languageserver_1.TextEdit.del(range)]), vscode_languageserver_1.CodeActionKind.QuickFix);
            ...
            for (const diagnostic of diagnostics) {
                if (diagnostic.message === 'Incorrect type. Expected "boolean".') {
                    ...
                    if (value === '"true"' || value === '"false"' || value === "'true'" || value === "'false'") {
                        ...
                        results.push(vscode_languageserver_1.CodeAction.create('Convert to boolean', createWorkspaceEdit(document.uri, [vscode_languageserver_1.TextEdit.replace(diagnostic.range, newValue)]), vscode_languageserver_1.CodeActionKind.QuickFix));
                    }
                }
            }
            return results;
