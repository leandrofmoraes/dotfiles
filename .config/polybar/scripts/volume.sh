...
function send_notification {
    ...
    if [[ $volume -eq 0 ]]
        then
            icon_name="~/.ricing/nov.png"
        elif [[ $volume -lt 35 ]] 
        then
            icon_name="~/.ricing/vlow.png"
        elif [[ $volume -lt 70 ]]
        then 
            icon_name="~/.ricing/vmid.png"
        elif [[ $volume -lt 100 ]]
        then 
            icon_name="~/.ricing/vhigh.png"
        else
            icon_name="~/.ricing/vthigh.png"
    fi;
    bn=$(( (volume + 5) / 5 ))
...
case $1 in
	...
	if is_mute ; then
    DIR=`dirname "$0"`
    $DIR/notify-send.sh -i "~/.ricing/mute.png" --replace=555 -u normal "Volume: Mute" -t 1000 -u critical
	else
	    send_notification
	fi
	;;
esac
#mpg123 ~/.ricing/laser.mp3
